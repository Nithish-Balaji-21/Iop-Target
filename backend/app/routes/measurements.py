...
def get_baseline_iop(patient_id: int, db: Session = Depends(get_db)):
    ...
    if not first_measurement:
        # Return defaults instead of raising error for better UX
        return {
            "patient_id": patient_id,
            "baseline_iop_od": 21,
            "baseline_iop_os": 21,
            "source": "default",
            "message": "No measurements found. Using default baseline IOP (21 mmHg)."
        }
    ...
    return {
        ...
        "first_measurement_date": first_measurement.measurement_date.isoformat() if first_measurement.measurement_date else None,
